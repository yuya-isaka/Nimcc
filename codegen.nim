...
proc genAddr(node: Node) =
  case node.kind:
  of NdLvar:
    echo fmt"  lea rax, [rbp-{node.arg.offset}]"  #! アドレス計算を行うが，メモリアクセスは行わず，アドレス計算の結果そのもの(アドレス）をraxに代入
...
proc gen(node: Node) =
  ...
  of NdBlock:                       #todo {}の中身はひたすら生成, body: seq[Node]
    for tmp in node.body:
      gen(tmp)                      #! ExprStmt()にラップされてるから，スタックトップに値は残らない
  ...
  of NdFuncall:
    ...
    var i = nargs - 1
    while i >= 0:
      echo fmt"  pop {argreg[i]}"   #! 順番にスタックからPOPされている．(ABI仕様), RSPが関数の開始位置(RBP)まで戻る
      dec(i)
...
proc codegen*(prog: Function) =       # 完成形アセンブリ出力関数
  ...
  while fn != nil:
    ...
    var node = fn.node                # プログラムに入ってるノードが尽きるまでアセンブリ生成(連結リストだからこの書き方ができる)
    while node != nil:
      gen(node)
      node = node.next
