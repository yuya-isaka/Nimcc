...
proc genAddr(node: Node) =                                # 左辺値生成（アドレス返すってこと）, C言語の左辺値は基本的にメモリのアドレスを指定する式
  ...
  of NdLvar:
    ...
    return
  of NdDeref:
    gen(node.lhs)                                         # int *p = 3 のようにデリファレンス経由で値を代入するときに対応するため， pのアドレスが生成されるように左辺値をコンパイル
    return
...
proc gen(node: Node) =
  ...
  #? 計算フェーズ（returnせず，スタックに保存するだけ）
  case node.kind                                          
  of NdAdd:
    if node.ty.base != nil:                                                         # ポインタだったら，型のサイズに合わせたオフセットを求める，　ポインタの加減算はオフセットの分進める
